...
router.delete("/:id", async (req, res) => {
  try {
    ...
    // pull `code` so we can also clean up notifications tied to this course's tag
    ...
    // notifications don't have a foreign key to courses.id
    // (only a course_tag string), so cascade won't handle them.
    // clean them up here.
    if (course.code) {
      const { error: notifErr } = await supabase
        ...
        .delete()
        .eq("user_id", req.userId)
        .eq("course_tag", course.code);
      if (notifErr) {
        console.warn("Delete notifications error:", notifErr.message);
        // non-fatal, still proceed with course delete
      }
    }
